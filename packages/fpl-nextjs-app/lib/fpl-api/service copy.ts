    ...
    async updatePlayerStats(gameweekId: number): Promise<boolean> {
        try {
            ...
            if (liveData && liveData.elements) {
                ...
                // Verify if liveData.elements is an array as per FPL API schema
                if (Array.isArray(liveData.elements)) {
                    ...
                    for (const elementDetail of liveData.elements) {
                        ...
                        if (stats && stats.minutes > 0) { // Ensure stats object exists
                            playerStats.push({
                                player_id: elementDetail.id, // Use the numeric 'id' directly
                ...
                } else if (typeof liveData.elements === 'object' && liveData.elements !== null) {
                    // Original logic: Assume it's an object map if not an array (and is an object)
                    console.warn(`liveData.elements for Gameweek ${gameweek.id} is an object, not an array. Proceeding with Object.entries(). Ensure this is intended if fplApiService transforms the data.`);
                    for (const [elementId, data] of Object.entries(
                        liveData.elements as Record<string, { stats: any }> // Type assertion
                    )) {
                    ...
                    console.warn(`liveData.elements for Gameweek ${gameweek.id} has an unexpected structure: ${typeof liveData.elements}. Skipping stats processing for this gameweek.`);
                }

                // ... (rest of the batch insertion logic) ...
