...
import {
    ...
    ChatMessage,
} from '../../utils/claude/context-manager';
import { shouldUseTool } from '../../utils/claude/tool-strategy';
import {
    withAnthropicTimeout,
    withToolTimeout,
} from '../../utils/claude/timeout-manager';
...
async function handleToolCalls(
    ...
    await Promise.all(
        toolCalls.map(async (toolCall) => {
            ...
            } = validateAndFixToolParameters(toolCall.name, toolCall.input);
        ...
        if (streamingEnabled && streamHandler) {
            ...
            if (!streamResult.success) {
                return {
                    ...
                    chatId: assignedChatId || '',
                    answer:
                        streamResult.error || 'Error processing your request',
            ...
            if (toolCalls.length > 0) {
                ...
                answer =
                    textBlock?.text ||
                    'Could not extract a text response after tool use.';
                ...
                answer =
                    textBlock?.text ||
                    'No tool calls were made, and no direct text response was found.';
            }
        }
