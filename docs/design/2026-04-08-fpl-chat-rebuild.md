...
```sql
-- already exist in current schema, preserved
...
messages (id, chat_id, role, content, tool_calls, tool_results, run_id, created_at)

-- NEW: durable run record with run_id as idempotency key
chat_runs (
  ...
  assistant_message_content text,              -- filled in as streaming progresses
  tool_events jsonb NOT NULL DEFAULT '[]'::jsonb, -- append-only log of tool calls + results during the run
...
**Run-level durability + idempotency (design-critical).** The `run_id` UUID is generated by the browser before the request is sent. The server's handling of every `POST /agent/run` is:

1. **On entry (before any Anthropic call):** `INSERT INTO chat_runs (run_id, chat_id, user_id, status, user_message_content) VALUES (..., 'pending', ...) ON CONFLICT (run_id) DO NOTHING RETURNING run_id`.
   - If the insert happened, the server owns this run and continues.
   - If `ON CONFLICT` fired, this is a retry of an in-flight or completed run. The server reads the existing row: if `status='completed'`, it replays the stored `assistant_message_content` + `tool_events` as AG-UI events (no new Anthropic call); if `status='streaming'`, it waits for the in-flight run or returns the current partial state; if `status='failed'`, it returns the stored error.
2. **During streaming:** `UPDATE chat_runs SET status='streaming', updated_at=now() WHERE run_id=$1` at start. Tool call events append to `tool_events` JSONB via `jsonb_set`/`||` operators in batched UPDATEs (not one-per-event — too chatty for Postgres; batch every 500ms or on tool boundary).
3. **On completion:** `UPDATE chat_runs SET status='completed', assistant_message_content=$final_text, completed_at=now(), updated_at=now() WHERE run_id=$1`. Then in the same transaction, insert a `messages` row with `role='assistant'`, `run_id=$1`, and the final content + tool_calls/tool_results JSONB (for eval replay).
4. **On failure:** `UPDATE chat_runs SET status='failed', error=$err, updated_at=now() WHERE run_id=$1`. No assistant message written.

**What this buys us:**
- **Client disconnect mid-stream** → the run row survives. Next poll/retry with the same `run_id` resumes or replays.
- **Backend restart** → the run row is `status='streaming'` when the process died. A sweeper job (out of scope for Phase 1, tracked in risks) can mark stale `streaming` runs as `failed` after a timeout. For Phase 1 we document the limitation and expect the frontend to show "run was interrupted" to the user.
- **Client retry with the same `run_id`** → `ON CONFLICT DO NOTHING` prevents duplicate execution, and the existing row's state is returned. The browser is responsible for generating stable `run_id` UUIDs and retrying with the same ID on failure.
- **Double-click submit** → ditto; second request hits the same row, is idempotent.

Note: `messages.tool_calls` and `messages.tool_results` remain JSONB columns so we can replay conversations exactly (including tool evidence) in the eval harness. `messages.run_id` is a foreign key into `chat_runs` so each persisted message can be traced back to the run that produced it.
...
2. **Stale `streaming` runs after backend crash.** (Related to chat persistence design in §7.) If the agent server crashes mid-stream, the `chat_runs` row stays in `status='streaming'` forever. Phase 1 has no automatic sweeper — the frontend shows the user "this run was interrupted" when they reload the chat and sees a stale streaming row older than ~5 minutes. **Phase 2b or later:** add a sweeper job that marks stale `streaming` runs as `failed` after a configurable timeout.
...
9. **Supabase RLS + Python agent writes.** The Python backend needs to write `chats`, `messages`, and `chat_runs` rows under the user's identity so RLS policies allow the insert. Approach: forward the user's JWT to the Supabase Python client, which passes it as the `Authorization` header on database requests. This needs to be verified with an actual RLS-enforced insert in M4 before streaming goes live.
